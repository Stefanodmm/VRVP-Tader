...
def guardar_en_csv(tipo_orden, precio_actual, va_superior, va_inferior):
    """Guarda la información de la orden en un archivo CSV en la carpeta vrvb"""
    archivo_csv = os.path.join(os.path.dirname(__file__), 'ordenes.csv')  # Ruta del archivo CSV en la carpeta vrvb
...
def crear_csv():
    """Crea el archivo CSV si no existe en la carpeta vrvb"""
    archivo_csv = os.path.join(os.path.dirname(__file__), 'ordenes.csv')  # Ruta del archivo CSV en la carpeta vrvb
...
def mostrar_datos_tiempo_real():
    ...
    while True:
        try:
            ...
            # Imprimir distancias solo después de la actualización
...
def obtener_temporalidad():
    """Obtiene la temporalidad desde el archivo JSON y la convierte a segundos."""
    global espera  # Declarar que vamos a usar la variable global
    intervalo = config['trading']['interval']
    
    # Convertir el intervalo a segundos
    if isinstance(intervalo, int):
        espera = intervalo  # Si ya es un entero, asignarlo directamente
    elif intervalo.endswith('s'):
        espera = int(intervalo[:-1])  # Segundos
    elif intervalo.endswith('m'):
        espera = int(intervalo[:-1]) * 60  # Minutos
    elif intervalo.endswith('h'):
        espera = int(intervalo[:-1]) * 3600  # Horas
    elif intervalo.endswith('d'):
        espera = int(intervalo[:-1]) * 86400  # Días
    elif intervalo.endswith('S'):
        espera = int(intervalo[:-1]) * 604800  # Semanas
    elif intervalo.endswith('M'):
        espera = int(intervalo[:-1]) * 2592000  # Meses (aproximado)
    else:
        raise ValueError("Intervalo no válido. Usa 's', 'm', 'h', 'd', 'S' o 'M'.")


if __name__ == "__main__":
    obtener_temporalidad()
